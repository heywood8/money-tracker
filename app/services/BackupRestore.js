...
export const createBackup = async () => {
  try {
    ...
    const [accounts, categories, operations, budgets, appMetadata, balanceHistory, plannedOperations, merchantRules, notificationTemplates, budgetPlans, budgetPlanLines, budgetPlanLineCategories, budgetPlanLineGroups] = await Promise.all([
      ...
      // User-defined parse templates (migration 0024). Guarded so backups of
      // pre-0024 databases don't fail. These are hand-built by the user and
...
const importBackupSQLite = async (fileUri, cancelToken) => {
  ...
  try {
    ...
    // Parse templates may not exist in pre-0024 backups. Same reasoning as the
