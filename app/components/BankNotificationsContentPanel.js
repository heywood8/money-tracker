...
/**
 * Settings subpanel for bank-notification processing.
 *
 * Lets the user toggle the feature, runs the ingestion pipeline, and presents
 * the review queue: each unmatched notification gets an account + category
 * picker, and saving creates the operation while learning the bindings.
 */
export default function BankNotificationsContentPanel({ bottomInset }) {
  ...
  // On mount: read the flag, process once (if enabled), then load the queue.
  useEffect(() => {
    let cancelled = false;
    (async () => {
      setLoading(true);
      try {
        ...
        if (!cancelled) await reloadPending();
      } catch (error) {
...
BankNotificationsContentPanel.propTypes = {
  bottomInset: PropTypes.number,
};

BankNotificationsContentPanel.defaultProps = {
  bottomInset: 0,
};
