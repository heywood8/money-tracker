...
export default function SimpleTabs() {
  ...
  // conditional); only the first TABS.length are used at any time. Five is the
  // current maximum tab count: Operations, Graphs, Budget?, Accounts?, Settings.
  ...
  const orderedScreens = useMemo(() => {
    ...
    return list;
  }, [showAccountsTab, showBudgetTab, operationsScreen, accountsScreen, graphsScreen, budgetScreen, settingsScreen]);
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top', 'left', 'right']}>
      ...
      <SafeAreaView edges={['bottom']} style={styles.floatingBarWrapper} testID="tab-bar-wrapper">
        <View style={[
          styles.floatingBar,
          // Widen the bar for the extra tabs so labels don't get cramped. With the
          // Planned tab removed the max tab count dropped from 6 to 5, so the
          // thresholds shift down by one to keep each layout proportioned as before.
          TABS.length >= 4 && styles.floatingBarWide,
          TABS.length >= 5 && styles.floatingBarExtraWide,
          {
...
const styles = StyleSheet.create({
  ...
  screensContainer: {
    flex: 1,
    flexDirection: 'row',
    // Base width; overridden inline to SCREEN_WIDTH * TABS.length (3 to 5 tabs).
